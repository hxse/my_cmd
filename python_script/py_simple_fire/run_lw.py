...
def loop_whisper(
    ...
    """
    enable1 run whisper
    enable2 run translate
    enable3 run create apkg
    --lang {en,ja}
    """
    ...
    command += f"--whisper-name {whisperName} --import-anki {import_anki} --enable-release-apkg {import_anki} --anki-app {ankiPath} --initial-prompt {prompt} --enable_zip {enable_zip} --lang {lang}"
    print(command)
