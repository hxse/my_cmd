...
def update():
    ...
    result = [[k, v] for k, v in enumerate(word_match(option))]
...
def set_answer(event):
    ...
    if len(show_opt) > 0:
        event.app.exit(
            result={
                "value": option[show_opt[show_select + offset]],
                "index": show_opt[show_select + offset],
            }
        )
...
def run_app_list():
    ...
    option = [f"{k} {v}" for k, v in enumerate(config_option)]
    show_opt = [k for k, v in enumerate(option)]
    app = Application(
        layout=layout,
        full_screen=True,
