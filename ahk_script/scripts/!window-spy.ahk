...
Global oGui
Global oGuiCtrl_FollowMouse
Global oGuiCtrl_SimpleMethodForCtrlUnderMouse

WinSpyGui()

_GuiCtrl_FollowMouse_onClick(oThisCtrl, info) {
    _GuiCtrl_SimpleMethodForCtrlUnderMouse_setEnabled_asCtrlFollowMouse()
}

_GuiCtrl_FollowMouse_toggleValue() {
    oGuiCtrl_FollowMouse.Value := oGuiCtrl_FollowMouse.Value ? 0 : 1
    _GuiCtrl_SimpleMethodForCtrlUnderMouse_setEnabled_asCtrlFollowMouse()
}

_GuiCtrl_SimpleMethodForCtrlUnderMouse_setEnabled_asCtrlFollowMouse() {
    oGuiCtrl_SimpleMethodForCtrlUnderMouse.Enabled := oGuiCtrl_FollowMouse.Value
}
...
WinSpyGui() {
    ...
    oGuiCtrl_FollowMouse := oGui.Add("Checkbox", "xm w420 Left vCtrl_FollowMouse", "Follow Mouse (AltGr + m)")
    oGuiCtrl_FollowMouse.Value := vStartWithFollowMouse
    oGuiCtrl_FollowMouse.OnEvent("Click", _GuiCtrl_FollowMouse_onClick)
    oGui.Add("Text", "xm", "Window Title, Class and Process:")
    oGui.Add("Edit", "xm y+0 w420 r5 ReadOnly -Wrap vCtrl_Title")
    oGui.Add("Text", "xm", "Mouse Position")
    oGui.Add("Edit", "xm y+0 w420 r4 ReadOnly vCtrl_MousePos")
    oGui.Add("Text", "xm w420 vCtrl_CtrlLabel", (txtFocusCtrl := "Focused Control") ":")
    oGuiCtrl_SimpleMethodForCtrlUnderMouse := oGui.Add("Checkbox", "y+0 xm+5 Left vCtrl_SimpleMethodForCtrlUnderMouse" . (vStartWithFollowMouse ? "" : " Disabled"), "Simple Method for Control Under Mouse")
    oGuiCtrl_SimpleMethodForCtrlUnderMouse.Value := 0
    oGui.SetFont("s8")
    oGui.Add("Text", "y+0 xm+10 w410 cGray", "The simple method correctly retrieves the active/topmost child window of an application with a Multiple Document Interface (MDI, e.g. with tabs). However, it is less accurate for other purposes such as detecting controls inside a GroupBox control.")
    oGui.SetFont()
    oGui.Add("Edit", "y+2 xm w420 r4 ReadOnly vCtrl_Ctrl")
    oGui.Add("Text", "xm", "Active Window Postition:")
    oGui.Add("Edit", "y+0 xm w420 r2 ReadOnly vCtrl_Pos")
    oGui.Add("Text", "xm", "Status Bar Text:")
    oGui.Add("Edit", "y+0 xm w420 r2 ReadOnly vCtrl_SBText")
    oGui.Add("Text", "y+5 xm", "Window Text:")
    oGui.Add("Checkbox", "y+0 xm+5 vCtrl_IsSlow", "Slow TitleMatchMode")
    oGui.SetFont("s8")
    oGui.Add("Text", "y+0 xm+10 w410 cGray", "TitleMatchMode speeds:")
    oGui.Add("Text", "y+0 xm+20 w400 cGray", "Fast:")
    oGui.Add("Text", "yp xm+50 w370 cGray", "(default behavior) Certain controls' text (e.g. of Edit controls) may not be detected.")
    oGui.Add("Text", "y+0 xm+20 w400 cGray", "Slow:")
    oGui.Add("Text", "yp xm+50 w370 cGray", "Works with more controls, but can be extremely slow if any applications are busy or `"not responding`".")
    oGui.Add("Text", "y+0 xm+10 w410 cGray", "(WinGetText and ControlGetText (probably?) also use the TitleMatchMode speed for its WinText and ExcludeText params, BUT always use the slow mode to retrieve the text.)")
    oGui.SetFont()
    oGui.Add("Text", "y+3 xm+5", "Visible Text:")
    oGui.Add("Edit", "y+0 xm w420 r7 ReadOnly vCtrl_VisText")
    oGui.Add("Text", "y+2 xm+5", "Hidden Text:")
    oGui.Add("Edit", "y+0 xm w420 r7 ReadOnly vCtrl_HiddenText")
    ; oGui.Add("Text",,"All Text:")
    ; oGui.Add("Edit","w420 r7 ReadOnly vCtrl_AllText")
    oGui.Add("Text", "xm w420 r1 vCtrl_Freeze", (txtNotFrozen := "(Hold Ctrl or Shift to suspend updates)"))
...
WinSpySize(GuiObj, MinMax, Width, Height) {
    ...
    list := "Title,MousePos,Ctrl,Pos,SBText,VisText,HiddenText,Freeze"
    Loop Parse list, ","
...
TryUpdate() {
    ...
    Ctrl_SimpleMethodForCtrlUnderMouse := oGui["Ctrl_SimpleMethodForCtrlUnderMouse"].Value
    Ctrl_FollowMouse := oGui["Ctrl_FollowMouse"].Value
    CoordMode "Mouse", "Screen"
    MouseGetPos &msX, &msY, &msWin, &msCtrl, 2 + Ctrl_SimpleMethodForCtrlUnderMouse ; get ClassNN and hWindow
    actWin := WinExist("A")
...
#HotIf WinActive(oGui.Hwnd)
<^>!m:: _GuiCtrl_FollowMouse_toggleValue()
Esc:: WinSpyClose(oGui)